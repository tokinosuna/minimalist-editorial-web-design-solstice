...
function updateHeaderIndicator(newSectionId) {
    if (!currentIndicator || isHeaderAnimating || currentIndicator.dataset.section === newSectionId) return;
...
if (logoTrigger) logoTrigger.addEventListener('click', (e) => { e.preventDefault(); openMeaningModal(); });
if (meaningModalClose) meaningModalClose.addEventListener('click', closeMeaningModal);
if (meaningModalOverlay) meaningModalOverlay.addEventListener('click', closeMeaningModal);
...
// gsap.context is great for managing animations and cleanup.
gsap.context(() => {
    ...
    if (window.matchMedia("(pointer: fine)").matches) {
        ...
        if (landingSection) {
            ...
            landingSection.addEventListener('mousemove', (e) => {
                ...
                if (heroImage) gsap.to(heroImage, { x: x * 20, y: y * 20, rotationY: x * 10, rotationX: -y * 10, duration: 0.5, ease: 'power2.out' });
                if (parallaxTexts.length >= 3) {
